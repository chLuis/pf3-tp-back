...
const buscarProductos = async (name) => {
  try {
...
const productosRelacionados = async (id_categoria) => {
  const connection = await db_connection;
  const [result] = await connection.query(
    `SELECT  p.id_producto,
              p.nombre,
...
const carritoService = async (carrito) => {
  if(carrito.length === 0) {
    const response = {
      status: 400,
  ...
  let queryCarrito = 'p.id_producto = '
  for(item in carrito) {
    queryCarrito += carrito[item] + ' OR p.id_producto = '
  }
  queryCarrito = queryCarrito.slice(0, -20)
  try {
    const connection = await db_connection;
    const [results] = await connection.query(`
      SELECT  p.id_producto,
    ...
    const response = {
      status: 400,
      message: "Fallo en la recoleccion del carrito",
      data: error
    };
    return response
  }
